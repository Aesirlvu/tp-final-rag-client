...
import { useAllVectorsFor3D } from "./hooks/useRetriever";
...
// Función para convertir datos de Pinecone a VectorPoint con PCA
const convertPineconeDataToVectorPoints = (
  pineconeData: any
): VectorPoint[] => {
  if (!pineconeData?.vectors || pineconeData.vectors.length === 0) {
    return [];
  }

  try {
    logger.debug(
      `Converting ${pineconeData.vectors.length} vectors to 3D points`
    );

    // Aplicar PCA para reducir a 3D
    const pca = new PCA(pineconeData.vectors);
    const reducedVectors = pca
      .predict(pineconeData.vectors, { nComponents: 3 })
      .to2DArray();

    // Convertir a VectorPoint
    const vectorPoints: VectorPoint[] = reducedVectors.map(
      (position, index) => ({
        id: `vector-${index}`,
        position: [position[0] * 10, position[1] * 10, position[2] * 10] as [
          number,
          number,
          number
        ...
        metadata: {
          originalIndex: index,
          namespace: pineconeData.namespace || "unknown",
          vectorLength: pineconeData.vectors[index]?.length || 0,
          pcaComponents: position,
        },
        namespace: pineconeData.namespace,
      })
    );

    logger.debug(
      `Successfully converted ${vectorPoints.length} vectors to 3D points`
    );
    return vectorPoints;
  } catch (error) {
    logger.error("Error converting Pinecone data to VectorPoints", error);
    return [];
  }
};
...
const MedicalRAGVisualizer: React.FC = () => {
  ...
  // Usar hook para obtener datos de Pinecone
  const { data: pineconeData, isLoading, error } = useAllVectorsFor3D();
  ...
  const points = useMemo(() => {
    // Si tenemos datos de Pinecone, convertirlos
    if (pineconeData?.vectors && pineconeData.vectors.length > 0) {
      return convertPineconeDataToVectorPoints({
        vectors: pineconeData.vectors,
        totalSampledVectors: pineconeData.totalSampledVectors,
        namespacesCount: pineconeData.namespacesCount,
        maxVectorsPerNamespace: pineconeData.maxVectorsPerNamespace,
      });
    }
  ...
  const stats = useMemo(
    () => ({
      entities: points.filter((p) => p.type === "vector").length,
      ...
      <div className="bg-card border-b border-border shrink-0">
        <div className="px-4 pt-4 pb-2">
          <h1 className="text-2xl font-bold text-foreground">
            Visualizador 3D - Vectores Pinecone
          </h1>
          <p className="text-sm text-muted-foreground">
            {pineconeData
              ? `PCA 3D de ${pineconeData.totalSampledVectors} vectores muestreados de ${pineconeData.namespacesCount} namespaces`
              : "Visualización interactiva del espacio vectorial médico"}
          </p>
        </div>
...
export default DimensionsVisualizer;
